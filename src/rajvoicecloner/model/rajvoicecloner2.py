"""
RajVoiceCloner: A Tokenizer-free speech generation model

This module contains the main RajVoiceCloner model implementation, including configuration classes
and the core RajVoiceClonerModel for text-to-speech generation.

Copyright 2026 RajVoiceCloner
...
from ..modules.locdit import CfmConfig, UnifiedCFM, RajVoiceClonerLocDiTV2
from ..modules.locenc import RajVoiceClonerLocEnc
...
class RajVoiceClonerEncoderConfig(BaseModel):
    hidden_dim: int = 1024
    ffn_dim: int = 4096
    num_heads: int = 16
    num_layers: int = 4
...
class RajVoiceClonerDitConfig(BaseModel):
    hidden_dim: int = 1024
    ffn_dim: int = 4096
    num_heads: int = 16
    num_layers: int = 4
...
class RajVoiceClonerConfig(BaseModel):
    ...
    encoder_config: RajVoiceClonerEncoderConfig
    dit_config: RajVoiceClonerDitConfig
...
class LoRAConfig(BaseModel):
    ...
    enable_dit: bool = False  # Apply LoRA to RajVoiceClonerLocDiT
    ...
    # Projection layer attribute names to find on RajVoiceCloner2Model
    target_proj_modules: list[str] = ["enc_to_lm_proj", "lm_to_dit_proj", "res_to_dit_proj", "fusion_concat_proj"]


RajVoiceClonerConfig.model_rebuild()


class RajVoiceCloner2Model(nn.Module):
    def __init__(
        self,
        config: RajVoiceClonerConfig,
        ...
        if resolved_dtype != self.config.dtype:
            print(
                f"[rajvoicecloner2] adjusted dtype {self.config.dtype} -> {resolved_dtype} for device {self.device}",
                file=sys.stderr,
            )
        ...
        self.feat_encoder = RajVoiceClonerLocEnc(encoder_config, input_dim=config.feat_dim)
        ...
        self.feat_decoder = UnifiedCFM(
            ...
            estimator=RajVoiceClonerLocDiTV2(decoder_config, in_channels=config.feat_dim),
    ...
    def optimize(self, disable: bool = False):
        ...
        try:
            if self.device != "cuda":
                raise ValueError("RajVoiceClonerModel can only be optimized on CUDA device")
            try:
    ...
    def from_local(
        ...
        with open(os.path.join(path, "config.json"), "r", encoding="utf-8") as _cfg_f:
            config = RajVoiceClonerConfig.model_validate_json(_cfg_f.read())
