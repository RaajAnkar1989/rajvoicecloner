...
from ..modules.locdit import CfmConfig, UnifiedCFM, RajVoiceClonerLocDiT
...
class LoRAConfig(BaseModel):
    ...
    # Projection layer attribute names to find on RajVoiceClonerModel
...
class RajVoiceClonerModel(nn.Module):
    def __init__(
        self,
        ...
        if resolved_dtype != self.config.dtype:
            print(
                f"[rajvoicecloner] adjusted dtype {self.config.dtype} -> {resolved_dtype} for device {self.device}",
                file=sys.stderr,
            )
        ...
        self.feat_decoder = UnifiedCFM(
            ...
            estimator=RajVoiceClonerLocDiT(decoder_config, in_channels=config.feat_dim),
