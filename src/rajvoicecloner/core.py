...
from .model.rajvoicecloner import RajVoiceClonerModel, LoRAConfig
...
class RajVoiceCloner:
    def __init__(
        self,
        rajvoicecloner_model_path: str,
        ...
        """Initialize RajVoiceCloner TTS pipeline.

        Args:
            rajvoicecloner_model_path: Local filesystem path to the RajVoiceCloner model assets
            ...
            device: Runtime device. If set to ``None`` or ``"auto"``, RajVoiceCloner
        ...
        """
        print(
            f"rajvoicecloner_model_path: {rajvoicecloner_model_path}, zipenhancer_model_path: {zipenhancer_model_path}, enable_denoiser: {enable_denoiser}",
            file=sys.stderr,
        )
        ...
        config_path = os.path.join(rajvoicecloner_model_path, "config.json")
        with open(config_path, "r", encoding="utf-8") as f:
            config = json.load(f)
        arch = config.get("architecture", "rajvoicecloner").lower()
        arch = {LEGACY_ARCH_V1: "rajvoicecloner", LEGACY_ARCH_V2: "rajvoicecloner2"}.get(arch, arch)

        if arch == "rajvoicecloner2":
            self.tts_model = RajVoiceCloner2Model.from_local(
                rajvoicecloner_model_path,
                optimize=optimize,
                device=device,
                lora_config=lora_config,
            )
            print("Loaded RajVoiceCloner2Model", file=sys.stderr)
        elif arch == "rajvoicecloner":
            self.tts_model = RajVoiceClonerModel.from_local(
                rajvoicecloner_model_path,
                optimize=optimize,
                device=device,
                lora_config=lora_config,
            )
            print("Loaded RajVoiceClonerModel", file=sys.stderr)
        else:
        ...
        if enable_denoiser and zipenhancer_model_path is not None:
            ...
            self.denoiser = None
        if optimize:
            print("Warm up RajVoiceClonerModel...", file=sys.stderr)
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        """Instantiate ``RajVoiceCloner`` from a Hugging Face Hub snapshot.
        ...
        Kwargs:
            Additional keyword arguments passed to the ``RajVoiceCloner`` constructor.

        Returns:
            RajVoiceCloner: Initialized instance whose ``rajvoicecloner_model_path`` points to
        ...
        return cls(
            rajvoicecloner_model_path=local_path,
    ...
    def _generate(
        ...
        is_v2 = isinstance(self.tts_model, RajVoiceCloner2Model)
        if reference_wav_path is not None and not is_v2:
            raise ValueError("reference_wav_path is only supported with RajVoiceCloner2 models")
    ...
    # ------------------------------------------------------------------ #
    # LoRA Interface (delegated to RajVoiceClonerModel)
    # ------------------------------------------------------------------ #
