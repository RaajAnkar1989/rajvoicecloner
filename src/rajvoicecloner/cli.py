#!/usr/bin/env python3
"""
RajVoiceCloner Command Line Interface

RajVoiceCloner2-first CLI for voice design, cloning, and batch processing.
"""
...
LEGACY_ARCH_V1 = "vox" + "cpm"
LEGACY_ARCH_V2 = LEGACY_ARCH_V1 + "2"
...
def warn_legacy_mode():
    print(
        "Warning: legacy root CLI arguments are deprecated. Prefer `rajvoicecloner design|clone|batch ...`.",
        file=sys.stderr,
    )
...
def detect_model_architecture(args) -> str | None:
    ...
    if os.path.isdir(model_location):
        ...
        with open(config_path, "r", encoding="utf-8") as f:
            arch = json.load(f).get("architecture", "rajvoicecloner").lower()
            return {LEGACY_ARCH_V1: "rajvoicecloner", LEGACY_ARCH_V2: "rajvoicecloner2"}.get(arch, arch)

    model_hint = str(model_location).lower()
    if "rajvoicecloner2" in model_hint or LEGACY_ARCH_V2 in model_hint:
        return "rajvoicecloner2"
    if (
        "rajvoicecloner1.5" in model_hint
        or "rajvoicecloner-1.5" in model_hint
        or "rajvoicecloner_1.5" in model_hint
        or f"{LEGACY_ARCH_V1}1.5" in model_hint
        or f"{LEGACY_ARCH_V1}-1.5" in model_hint
        or f"{LEGACY_ARCH_V1}_1.5" in model_hint
    ):
        return "rajvoicecloner"
...
def validate_reference_support(args, parser):
    ...
    if arch == "rajvoicecloner":
        parser.error("--reference-audio is only supported with RajVoiceCloner2 models.")
...
def load_model(args):
    from rajvoicecloner.core import RajVoiceCloner

    print("Loading RajVoiceCloner model...", file=sys.stderr)
    ...
    if lora_weights_path:
        from rajvoicecloner.model.rajvoicecloner import LoRAConfig
    ...
    if args.model_path:
        try:
            model = RajVoiceCloner(
                rajvoicecloner_model_path=args.model_path,
    ...
    try:
        model = RajVoiceCloner.from_pretrained(
...
def cmd_validate(args, parser):
    from rajvoicecloner.training.validate import (
...
def cmd_serve(args, parser):
    # Lazy imports: keep CLI startup fast and avoid requiring server deps
    # for the non-server subcommands (matches load_model's lazy pattern).
    import uvicorn
    from rajvoicecloner.server.app import create_app
    from rajvoicecloner.server.branding import APP_NAME, API_KEY_ENV, LEGACY_API_KEY_ENV
    from rajvoicecloner.server.config import ServerConfig
    ...
    config = ServerConfig(
        ...
        api_key=args.api_key or os.environ.get(API_KEY_ENV) or os.environ.get(LEGACY_API_KEY_ENV) or None,
    )
    if args.llm_url:
    ...
    print(f"{APP_NAME} running at http://{args.host}:{args.port}", file=sys.stderr)
...
def _add_common_generation_args(parser):
    ...
    parser.add_argument(
        "--control",
        type=str,
        help="Control instruction for RajVoiceCloner2 voice design/cloning",
    )
    parser.add_argument(
        "--cfg-value",
        type=float,
...
def _add_prompt_reference_args(parser):
    ...
    parser.add_argument(
        "--reference-audio",
        "-ra",
        help="Reference audio for voice cloning (RajVoiceCloner2 only)",
    )
    parser.add_argument(
        "--denoise",
        action="store_true",
...
def _add_model_args(parser):
    parser.add_argument("--model-path", type=str, help="Local RajVoiceCloner model path")
    parser.add_argument(
        "--hf-model-id",
        type=str,
        default=DEFAULT_HF_MODEL_ID,
        help="Hugging Face repo id for compatible engine weights",
    )
    parser.add_argument(
        "--device",
        type=str,
...
def _build_parser():
    parser = argparse.ArgumentParser(
        description="RajVoiceCloner CLI - RajVoiceCloner2-first voice design, cloning, and batch processing",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  rajvoicecloner design --text "Hello world" --output out.wav
  rajvoicecloner design --text "Hello world" --control "warm female voice" --output out.wav
  rajvoicecloner clone --text "Hello" --reference-audio ref.wav --output out.wav
  rajvoicecloner batch --input texts.txt --output-dir ./outs --reference-audio ref.wav
        """,
    )
    ...
    design_parser = subparsers.add_parser(
        "design", help="Generate speech with RajVoiceCloner2-first voice design"
    )
    ...
    serve_parser = subparsers.add_parser(
        "serve",
        help="Run RajVoiceCloner: ElevenLabs-compatible REST API + web UI",
    )
    ...
    serve_parser.add_argument(
        "--api-key",
        type=str,
        help="Require this API key via the xi-api-key header (or env RAJVOICEASSISTANT_API_KEY)",
    )
    serve_parser.add_argument(
        "--data-dir",
        type=str,
        help="Directory for voices and history (default: ~/.rajvoicecloner/studio)",
    )
    serve_parser.add_argument(
        "--preload",
        action="store_true",
    ...
    serve_parser.add_argument(
        "--llm-url",
        type=str,
        help="OpenAI-compatible LLM base URL for smart agents (default: http://localhost:11434/v1, i.e. Ollama)",
    )
    serve_parser.add_argument(
        "--llm-model",
        type=str,
        help="LLM model name for smart agents (default: llama3.2)",
    )
    serve_parser.add_argument(
        "--llm-api-key",
        type=str,
        help="API key for the LLM endpoint, if it requires one",
    )
