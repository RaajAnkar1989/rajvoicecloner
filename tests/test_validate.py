...
# Stub rajvoicecloner package so imports work without full dependencies
pkg = types.ModuleType("rajvoicecloner")
pkg.__path__ = [str(ROOT / "src" / "rajvoicecloner")]
sys.modules.setdefault("rajvoicecloner", pkg)

training_pkg = types.ModuleType("rajvoicecloner.training")
training_pkg.__path__ = [str(ROOT / "src" / "rajvoicecloner" / "training")]
sys.modules.setdefault("rajvoicecloner.training", training_pkg)

from rajvoicecloner.training.validate import ValidationResult, validate_manifest
...
class TestValidateManifest:
    ...
    def test_cli_validate_exit_code(self, tmp_dir):
        ...
        proc = subprocess.run(
            [sys.executable, "-m", "rajvoicecloner.cli", "validate", "--manifest", str(manifest)],
            capture_output=True,
            text=True,
        )
