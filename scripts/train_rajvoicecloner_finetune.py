...
from rajvoicecloner.model import RajVoiceClonerModel, RajVoiceCloner2Model
from rajvoicecloner.model.rajvoicecloner import LoRAConfig as LoRAConfigV1
from rajvoicecloner.model.rajvoicecloner2 import LoRAConfig as LoRAConfigV2
from rajvoicecloner.training import (
    Accelerator,
    BatchProcessor,
    TrainingTracker,
    build_dataloader,
...
def train(
    ...
    hf_model_id: str = "",  # HuggingFace model ID (e.g., "RajVoiceCloner/RajVoiceCloner1.5")
    ...
    with open(os.path.join(pretrained_path, "config.json"), "r", encoding="utf-8") as _f:
        _arch = json.load(_f).get("architecture", "rajvoicecloner").lower()
    _model_cls = RajVoiceCloner2Model if _arch == "rajvoicecloner2" else RajVoiceClonerModel
    LoRAConfig = LoRAConfigV2 if _arch == "rajvoicecloner2" else LoRAConfigV1
    if accelerator.rank == 0:
    ...
    if max_batch_tokens and max_batch_tokens > 0:
        from rajvoicecloner.training.data import compute_sample_lengths
...
if __name__ == "__main__":
    from rajvoicecloner.training.config import load_yaml_config
