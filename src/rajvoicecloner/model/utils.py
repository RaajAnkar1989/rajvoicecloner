...
# Ref: https://github.com/RajVoiceCloner/RajVoiceCloner/issues/256#issuecomment-4235252732
...
def pick_runtime_dtype(device: str, configured_dtype: str) -> str:
    ...
    Users can override with ``RAJVOICECLONER_MPS_DTYPE`` (e.g. ``bfloat16``) when
    they want to test future MPS improvements.
    """
    if device != "mps":
        return configured_dtype

    override = os.environ.get("RAJVOICECLONER_MPS_DTYPE", "").strip().lower()
    if override:
        if override not in _VALID_DTYPE_OVERRIDES:
            raise ValueError(
                f"RAJVOICECLONER_MPS_DTYPE='{override}' is not one of "
