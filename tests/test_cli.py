...
CLI_PATH = ROOT / "src" / "rajvoicecloner" / "cli.py"
V1_MODEL_PATH = ROOT / "models" / "openbmb__RajVoiceCloner1.5"
V2_MODEL_PATH = ROOT / "models" / "RajVoiceCloner2-1B-newaudiovae-6hz-nope-sft"


pkg = types.ModuleType("rajvoicecloner")
pkg.__path__ = [str(ROOT / "src" / "rajvoicecloner")]
sys.modules.setdefault("rajvoicecloner", pkg)

core_stub = types.ModuleType("rajvoicecloner.core")


class StubRajVoiceCloner:
    pass


core_stub.RajVoiceCloner = StubRajVoiceCloner
sys.modules["rajvoicecloner.core"] = core_stub

spec = importlib.util.spec_from_file_location("rajvoicecloner.cli", CLI_PATH)
cli = importlib.util.module_from_spec(spec)
sys.modules["rajvoicecloner.cli"] = cli
...
def run_main(monkeypatch, argv):
    monkeypatch.setattr(sys, "argv", ["rajvoicecloner", *argv])
    cli.main()
...
def test_parser_defaults_to_rajvoicecloner2():
...
def test_load_model_respects_no_optimize_for_local_model(monkeypatch):
    ...
    class FakeRajVoiceCloner:
        def __init__(self, **kwargs):
            calls["kwargs"] = kwargs
            self.tts_model = DummyTTSModel()

    monkeypatch.setattr(core_stub, "RajVoiceCloner", FakeRajVoiceCloner)
...
def test_clone_rejects_reference_audio_for_v1_hf_model_id(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        ...
        [
            ...
            str(reference_audio),
            "--hf-model-id",
            "RajVoiceCloner/RajVoiceCloner1.5",
            "--output",
            str(tmp_path / "out.wav"),
        ],
    )
...
def test_detect_model_architecture_uses_local_configs():
    ...
    assert cli.detect_model_architecture(v1_args) == "rajvoicecloner"
    assert cli.detect_model_architecture(v2_args) == "rajvoicecloner2"
